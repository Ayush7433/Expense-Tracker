...
- **Dashboard**: Visual representations of expenses and aggregated statistics (e.g., total expenses, breakdown by category, top category). Includes a filter bar to toggle between "All Time" and a specific month-year, dynamically switching the trend chart between monthly and daily x-axes. Also includes a "Budget Health" summary with its own independent month selector to track categories nearing or over their limit.
...
- `client/`: Contains the frontend React application.
  ...
  - `src/components/`: Reusable UI components.
    ...
    - `dashboard/`: Dashboard-specific components (`StatsCard`, `RecentExpenses`, `ExpenseChart`, `MonthlyExpenseChart`, `BudgetHealth`, `DashboardFilterBar`).
...
- **Expense Module (`expenseController`, `expenseRoutes`, `Expense` model)**: Manages creating, fetching (with filtering, search, pagination), updating, and deleting expenses. Provides dashboard statistics via server-side aggregation, which supports an optional month parameter to return a daily breakdown and the top category for that specific month (or defaults to a trailing-12-month breakdown for all-time view). Checks projected budget impact before an expense is saved and refreshes budget status after any expense change.
...
- The `RecentExpenses` component maintains a strict horizontal flex layout (`flex-row`) even on mobile, pinning the expense amount to the right edge and wrapping secondary text (like date) underneath the amount, preventing the amount from being disconnected on its own line.
