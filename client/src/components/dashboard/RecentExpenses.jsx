      ...
      {expenses.length === 0 ? (
        ...
        <div className="space-y-4">
          {expenses.map((expense) => (
            <div
              key={expense._id}
              className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-2xl border border-gray-100 px-4 py-4 sm:px-5 transition-all hover:border-blue-200 hover:bg-blue-50/40 dark:border-slate-800 dark:hover:border-blue-900 dark:hover:bg-blue-950/30 w-full overflow-hidden"
            >
              {/* Left */}
              <div className="flex min-w-0 flex-1 items-center gap-3 sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span
                      className="inline-flex max-w-[80px] sm:max-w-[120px] rounded-full bg-blue-100 px-2 py-1 text-xs font-medium text-blue-700 dark:bg-blue-950/50 dark:text-blue-400"
                      title={expense.category}
                    >
                      <span className="truncate">{expense.category}</span>
                    </span>

                    <span className="truncate text-xs sm:text-sm text-gray-500 dark:text-slate-400 hidden sm:block">
              ...
              <div className="mt-2 sm:mt-0 shrink-0 text-right max-w-[80px] sm:max-w-[90px] sm:ml-2 md:max-w-[150px] lg:max-w-[200px]">
                <p
                  className="truncate text-base sm:text-lg font-bold text-gray-900 dark:text-white"
